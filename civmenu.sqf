...
if ((_loopart == "arrest") and (player distance prisonflag <= 80))  then 
...
if(stolencash > 0)then
	...
	["geld", -(_INVrmv)] call INV_AddInvItem;
	...
	//remove the blue dye/s from the bank robber 1 minute after he is jailed (in case he wants to see the evidence for himself)
	sleep 60;
...
while {true} do 
	...
	if (player Distance prison_logic >= 11) exitWith {_exitart = "ausbruch"};
