...

{
	...
	if(INV_shortcuts)then{INV_shortcuts=false; titletext["ALR keys off", "PLAIN DOWN"];[] execVM "actionsRemove.sqs"}else{INV_shortcuts=true; titletext["ALR keys on", "PLAIN DOWN"];[] execVM "actions.sqf"};};
	//L key
	case 38: 
	...
	for [{_i=1}, {_i < 3}, {_i=_i+1}] do
		...
		_atms   = nearestObjects [_posFind,["Man", "ATM"],2];
		_civ    = _men select 0;
		_atm	= _atms select 0;
