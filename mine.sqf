...
if(_item == "Shovel")then
	...
	for [{_i=0}, {_i < 4}, {_i=_i+1}] do 
		...
		if(shoveldur <= 0)exitwith{player groupchat "The shovel broke"; ["Shovel", -1] call INV_AddInvItem; shoveldur=20;}; 
...
if(_item == "Pickaxe")then
	...
	for [{_i=0}, {_i < 4}, {_i=_i+1}] do 
		...
		if(pickaxedur <= 0)exitwith{player groupchat "The pickaxe broke"; ["Pickaxe", -1] call INV_AddInvItem; pickaxedur=50;}; 
...
if(_item == "JackHammer")then
	...
	for [{_i=0}, {_i < 4}, {_i=_i+1}] do 
		...
		if(hammerdur <= 0)exitwith{player groupchat "The Jack Hammer broke"; ["Jack Hammer", -1] call INV_AddInvItem; hammerdur=100;}; 
		_avail = floor (INV_Tragfaehigkeit - (call INV_GetOwnWeight));
		totalamount = totalamount + _amount;
		if((totalamount*3) >= _avail)exitwith{totalamount = (_avail/3); player groupchat "maximum weight reached"; _i=6;};
		player groupchat format["You got %1 %2", _amount, _name];
